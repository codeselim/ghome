...
-- Used to link sensor types to condition types: That is to say, except the case of the first mandatory condition, which conditions/operators can we use on the value of a given device ?p
CREATE TABLE `sensor_types_condition_types` (
	`sensor_type_id` INT NOT NULL,
	`condition_type_id` INT NOT NULL,
	FOREIGN KEY(sensor_type_id) REFERENCES sensors_types(`id`),
	FOREIGN KEY(condition_type_id) REFERENCES condition_types(`id`),
	PRIMARY KEY(`sensor_type_id`, `condition_type_id`)
);
