...
function connectLocal (argument) {
	...
	localhost.on("end", function () { console.log("LOCAL connection ENDED, trying to reconnect in 1s"); setTimeout(connectLocal, 1000);})
	localhost.on("error", function () { console.log("LOCAL connection ERROR, trying to reconnect in 1s"); setTimeout(connectLocal, 1000);})
}
